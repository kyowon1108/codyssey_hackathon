...
all_images = sorted(list(image_dir.glob("*.jpg")) + list(image_dir.glob("*.JPG")) + list(image_dir.glob("*.png")) + list(image_dir.glob("*.PNG")))

# 5의 배수만 선택 (인덱스 기준: 0, 5, 10, 15, ...)
selected_images = [img for idx, img in enumerate(all_images) if idx % 5 == 0]

print(f"Total images: {len(all_images)}")
print(f"Selected images (every 5th): {len(selected_images)}")
